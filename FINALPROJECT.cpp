			...
			while (true)
			{
				...
				if (manage == " 1 ")
				{
					...
					for (int i = 0; i < len; i++)
					{
						...
						{
							cout << " \n\nSubject :\t " << endl;
							cout << obj.academic_records[i].CourseName << endl;
				...
				else if (manage == " 3 ")
				{
					...
					if (str == " y ")
					{
						...
						stu_container[obj.details.SetStatus()] = obj;
					}
				}
				else if (manage == " 0 ")
				{
...
void addStaff()
{
	...
	cout << sta_base_id + sta_cout << endl;
	temp.details.SetId(sta_base_id + sta_cout);
	temp.details.SetIndex(sta_cout);
	sta_cout++;
	string str;
	int num;
	...
	//pushing to array
	sta_container[temp.details.SetIndex()] = temp;
		...
		if (pass == 1)
		{
			...
			while (true)
			{
				...
				else if (edit == " 0 ")
				{
					...
					sta_container[obj.details.SetIndex()] = obj;
					break;
				}
			}
		}
...
void manageStaff()
{
	...
	if (sta_id != 0)
	{
		sta_id -= sta_base_id;
		///controls
		while (sta_id < 0 || sta_id >= sta_cout)
		{
...
void GpaCalc(student obj)
{
	...
	for (int i = 0; i < len; i++)
	{
		...
		{
			tch += temp_record[i].CreditHour;
			gpa += getGpaScore(temp_record[i].Grade) * temp_record[i].CreditHour;
		}
	}
